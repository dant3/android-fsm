...
/**
 * <p><b>Port of Android framework internal StateMachine.</b>
 * See <a href="https://android.googlesource.com/platform/frameworks/base/+/master/core/java/com/android/internal/util/StateMachine.java>original source code for reference</a>
 * </p>
 *
 ...
 * desirable to have <code>getName</code> return the the state mapper name instead,
...
public class StateMachine {
    ...
    private static class LogRec {
         ...
         * processed.
         */
        LogRec(final StateMachine sm, final Message msg, final String info, final State state, final State orgState,
               final State transToState) {
         ...
         */
        public void update(final StateMachine sm, final Message msg, final String info, final State state, final State orgState,
        ...
        public String toString() {
            ...
            sb.append(" what=");
            final String what = mSm != null ? mSm.getWhatToString(mWhat) : "";
    ...
    private static class LogRecords {
         ...
         */
        synchronized void setSize(final int maxSize) {
        ...
        synchronized void setLogOnlyTransitions(final boolean enable) {
         ...
         */
        synchronized LogRec get(final int index) {
         ...
         *
         */
        synchronized void add(final StateMachine sm, final Message msg, final String messageInfo, final State state,
                              final State orgState, final State transToState) {
            mCount += 1;
            if (mLogRecVector.size() < mMaxSize) {
                mLogRecVector.add(new LogRec(sm, msg, messageInfo, state, orgState, transToState));
            } else {
                final LogRec pmi = mLogRecVector.get(mOldestIndex);
                mOldestIndex += 1;
    ...
    private static class SmHandler extends Handler {
         ...
         */
        private class HaltingState extends AbstractState {
            @Override
         ...
         */
        private class QuittingState extends AbstractState {
            @Override
            public boolean processMessage(final Message msg) {
                return NOT_HANDLED;
            }
        ...
        public final void handleMessage(final Message msg) {
            if (!mHasQuit) {
                ...
                if (mDbg && mSm != null) mSm.log("handleMessage: E msg.what=" + msg.what);
         ...
         */
        private void performTransitions(final State msgProcessedState, final Message msg) {
            /**
             ...
             */
            final boolean recordLogMsg = mSm.recordLogRec(mMsg) && (msg.obj != mSmHandlerObj);
            ...
            if (destState != null) {
                ...
                while (true) {
                     ...
                     */
                    final StateInfo commonStateInfo = setupTempStateStackWithStatesToEnter(destState);
                    // flag is cleared in invokeEnterMethods before entering the target state
                    mTransitionInProgress = true;
                    invokeExitMethods(commonStateInfo);
                    final int stateStackEnteringIndex = moveTempStateStackToStateStack();
        ...
        private void completeConstruction() {
             ...
             */
            int maxDepth = 0;
            for (final StateInfo si : mStateInfo.values()) {
                int depth = 0;
         ...
         */
        private State processMsg(final Message msg) {
         ...
         */
        private void invokeExitMethods(final StateInfo commonStateInfo) {
            while ((mStateStackTopIndex >= 0)
                    && (mStateStack[mStateStackTopIndex] != commonStateInfo)) {
                final State curState = mStateStack[mStateStackTopIndex].state;
         ...
         */
        private void invokeEnterMethods(final int stateStackEnteringIndex) {
        ...
        private void moveDeferredMessageAtFrontOfQueue() {
            ...
            for (int i = mDeferredMessages.size() - 1; i >= 0; i--) {
                final Message curMsg = mDeferredMessages.get(i);
        ...
        private int moveTempStateStackToStateStack() {
            final int startingIndex = mStateStackTopIndex + 1;
         ...
         */
        private StateInfo setupTempStateStackWithStatesToEnter(final State destState) {
            /**
         ...
         */
        private StateInfo addState(final State state, final State parent) {
            if (mDbg) {
        ...
        private void removeState(final State state) {
            ...
            for (final StateInfo si : mStateInfo.values()) {
                if (si.parentStateInfo == stateInfo) {
                    isParent = true;
                    break;
                }
            }
         ...
         */
        private SmHandler(final Looper looper, final StateMachine sm) {
            super(looper);
            mSm = sm;
        ...
        private void setInitialState(final State initialState) {
            if (mDbg) mSm.log("setInitialState: initialState=" + initialState.getName());
            mInitialState = initialState;
        }
        /** @see StateMachine#transitionTo(State) */
        private void transitionTo(final State destState) {
            if (mTransitionInProgress) {
                Log.wtf(mSm.mName, "transitionTo called while transition already in progress to " +
                        mDestState + ", new target state=" + destState);
            }
            final State prevState = mDestState;
            mDestState = destState;
            if (mDbg) mSm.log("transitionTo: destState=" + mDestState.getName());

            if (transitionListener != null) {
                try {
                    transitionListener.onTransition(prevState, destState);
                } catch (final Exception e) {
                    if (mDbg) mSm.loge("Exception caught from transition listener", e);
                }
            }
        }
        /** @see StateMachine#deferMessage(Message) */
        private void deferMessage(final Message msg) {
            if (mDbg) mSm.log("deferMessage: msg=" + msg.what);
            /* Copy the "msg" to "newMsg" as "msg" will be recycled */
            final Message newMsg = obtainMessage();
            newMsg.copyFrom(msg);
        ...
        private boolean isQuit(final Message msg) {
        ...
        private void setDbg(final boolean dbg) {
            mDbg = dbg;
        }
    }
     ...
     */
    private void initStateMachine(final String name, final Looper looper) {
        mName = name;
     ...
     * @param name of the state machine
     */
    protected StateMachine(final String name) {
        mSmThread = new HandlerThread(name);
        mSmThread.start();
        final Looper looper = mSmThread.getLooper();
     ...
     * @param name of the state machine
     */
    protected StateMachine(final String name, final Looper looper) {
        initStateMachine(name, looper);
    }
    /**
     ...
     * @param name of the state machine
     */
    protected StateMachine(final String name, final Handler handler) {
     ...
     */
    protected void onPreHandleMessage(final Message msg) {
    }
    /**
     ...
     * @param msg The message that is being handled
     */
    protected void onPostHandleMessage(final Message msg) {
    }
    /**
     ...
     * @param parent the parent of state
     */
    public final void addState(final State state, final State parent) {
    ...
    public final Message getCurrentMessage() {
        ...
        final SmHandler smh = mSmHandler;
     ...
     */
    public final void transitionTo(final State destState) {
     ...
     */
    public final void deferMessage(final Message msg) {
     ...
     */
    protected void unhandledMessage(final Message msg) {
     ...
     */
    protected void haltedProcessMessage(final Message msg) {
    }
    /**
     ...
     */
    public final void setLogRecSize(final int maxSize) {
     ...
     */
    public final void setLogOnlyTransitions(final boolean enable) {
     ...
     */
    public final LogRec getLogRec(final int index) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return null;
    ...
    public final Collection<LogRec> copyLogRecs() {
        final Vector<LogRec> vlr = new Vector<LogRec>();
        final SmHandler smh = mSmHandler;
        if (smh != null) {
            for (final LogRec lr : smh.mLogRecords.mLogRecVector) {
                vlr.add(lr);
            }
        }
        return vlr;
     ...
     * @param string
     */
    public void addLogRec(final String string) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     */
    protected boolean recordLogRec(final Message msg) {
        return true;
    }
    /**
     ...
     */
    protected String getLogRecString(final Message msg) {
        return "";
    }
    /**
     * @return the string for msg.what
     */
    protected String getWhatToString(final int what) {
        return null;
    }
    /**
     ...
     * @return  A Message object from the global pool
     */
    public final Message obtainMessage(final int what, final int arg1) {
     ...
     * Message is ignored if state machine has quit.
     */
    public void sendMessageDelayed(final int what, final Object obj, final long delayMillis) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    public void sendMessageDelayed(final int what, final int arg1, final long delayMillis) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    public void sendMessageDelayed(final int what, final int arg1, final int arg2, final long delayMillis) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    public void sendMessageDelayed(final int what, final int arg1, final int arg2, final Object obj,
     ...
     * Message is ignored if state machine has quit.
     */
    public void sendMessageDelayed(final Message msg, final long delayMillis) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final int what) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final int what, final Object obj) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final int what, final int arg1) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final int what, final int arg1, final int arg2) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final int what, final int arg1, final int arg2, final Object obj) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Message is ignored if state machine has quit.
     */
    protected final void sendMessageAtFrontOfQueue(final Message msg) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
     ...
     * Protected, may only be called by instances of StateMachine.
     */
    protected final void removeMessages(final int what) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
    ...
    protected final void removeDeferredMessages(final int what) {
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
        final Iterator<Message> iterator = smh.mDeferredMessages.iterator();
     ...
     */
    protected final boolean hasDeferredMessages(final int what) {
        final SmHandler smh = mSmHandler;
     ...
     */
    protected final boolean hasMessages(final int what) {
        final SmHandler smh = mSmHandler;
        if (smh == null) return false;
     ...
     * */
    protected final boolean isQuit(final Message msg) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
     ...
     */
    public void setDbg(final boolean dbg) {
        // mSmHandler can be null if the state machine has quit.
        final SmHandler smh = mSmHandler;
        if (smh == null) return;
        smh.setDbg(dbg);
    ...
    public String toString() {
        ...
        final PrintWriter pr = new PrintWriter(sr);
        dump(null, pr, null);
        pr.flush();
        pr.close();
        return sr.toString();
     ...
     */
    protected void logAndAddLogRec(final String s) {
        addLogRec(s);
        log(s);
    }
    /**
     ...
     * @param s is string log
     */
    protected void logv(final String s) {
        Log.v(mName, s);
    }
    /**
     ...
     * @param s is string log
     */
    protected void logi(final String s) {
        Log.i(mName, s);
    }
    /**
     ...
     * @param s is string log
     */
    protected void logw(final String s) {
        Log.w(mName, s);
    }
    /**
     ...
     * @param s is string log
     */
    protected void loge(final String s) {
        Log.e(mName, s);
    }
    /**
     * Log with error attribute
     ...
     */
    protected void loge(final String s, final Throwable e) {
        Log.e(mName, s, e);
    }

    public void setTransitionListener(TransitionListener transitionListener) {
        mSmHandler.setTransitionListener(transitionListener);
    }
